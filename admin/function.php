...
//用户充值
function userdxchongzhi($uid,$jine,$cztype,$a=-1){
	global $con;
	$jine=floatval($jine);
	$inactiv=array(
			'uid' =>$uid,
...
function userdxchongzhi_sdcz($uid,$jine,$cztype,$shje,$beizhu,$iszs=0){
	global $con;
	$jine=floatval($jine);
	$iszs=intval($iszs);
	$inactiv=array(
...
function userdxchongzhi_sdkf($uid,$jine,$cztype,$shje,$beizhu){
	global $con;
	$jine=floatval($jine);
	$inactiv=array(
			'uid' =>$uid,
...
//存储操作日志 $uid客服id,$buid代理商id,$uname客服名称,$buname代理商名称,$cztype操作类型,$czmsg操作内容
function insertcaozuolog($uid,$buid,$uname,$buname,$cztype,$czmsg){
global $con;
	$msg="客服：".$uname." 给代理商 ".$buname."（uid：".$buid."）".$czmsg;
	$inarr=array(
			"uid"=>$uid,
			"buid"=>$buid,
			"uname"=>$uname,
	...
	$id=$con->insertabe("caozuolog",$inarr);
	return true;
}
