...
//参数说明 $sjh 手机号,$liuliang 要充值的流量,$tongdaoid 通道id,$qita 其他参数备用,&$err 错误信息填充变量
 function sendliuliang($sjh,$liuliang,$tongdaoid,$qita,&$err){
	...
	//获取发送通道
	$sql="select * from `liuliangtongdaolist` where id=".$tongdaoid." and zt=1";
	$re=$con->query($sql);
...
function sendliuliangf23($mobile,$package,$qita,&$err){
	...
	$userid="dx-hnxz";
	$pwd="A5CA2A06FB8EBA4F4B2071B5E890B922";
	...
	$validity="0";
	$times=date("YmdHis");
	$userkey=md5("userid".$userid."pwd".$pwd."orderid".$orderid."account".$account."gprs".$gprs."area".$area."effecttime".$effecttime."validity".$validity."times".$times."1C794B24E5F4B75CD1DF03B5B60144CB");
	
	
	$url="http://api.ejiaofei.net:11140/gprsChongzhiAdvance.do?"."userid=".$userid."&pwd=".$pwd."&orderid=".$orderid."&account=".$account."&gprs=".$gprs."&area=".$area."&effecttime=".$effecttime."&validity=".$validity."&times=".$times."&userkey=".$userkey;
...
function sendliuliangf51_getstatus(){
	...
	$logarr=array();
	while($row=mysql_fetch_array($re,MYSQL_ASSOC)){
		$sql="UPDATE liuliangdaili_log set istk=1,zt=2,upzttime=".time()." where id=".$row["id"];
		$con->query($sql);

		$uid=trim($row["uid"]);
		$shje=floatval($row["shje"]);
		$sql="UPDATE user_daili set dxnum=dxnum+".$shje." where id=".$uid;
		$con->query($sql);//更新用户余额
	}
	return false;
}
...
//广东省网
	return sendliuliangf65($mobiles,$packageSize,$qita,$err);
}
