...
function getusercznum($uid){
	...
	$sql="select SUM(shje) as num from `chongzhilog` where uid=".$uid." and zt=1 and cztype in(0,1,2)";
	$re=$con->query($sql);
	$row=mysql_fetch_array($re);
...
//用户充值
function userdxchongzhi($uid,$jine,$cztype){
	global $con;
	$jine=floatval($jine);
	$inactiv=array(
			'uid' =>$uid,
...
//获取发送验证码通道id
function gettongdaoid_yzm(){
	global $con;
	$sql="select id from `tongdaolist` where qt_jkyzmtd=1";
	$re=$con->query($sql);
	$row=mysql_fetch_array($re);
...
//用户充值 短信条数转金额
function userdxchongzhi_dxnum($uid,$dxnum){
	global $con;
	$sql="select jiage from `user` where id=".$uid;
	$re=$con->query($sql);
	$row=mysql_fetch_array($re);
...
function getsjhinfo($sjh){
    ...
    $header = array(
        'apikey: b011389504b0b2e68e131463dfecba26',
    );
    // 添加apikey到header
	...
	if((strlen($province)<=2) || (strlen($city)<=2)){
		...
		$rearr["province"]=$a["province"];
		$rearr["city"]=$a["city"];
	}
	if(empty($rearr["province"])){
			$rearr["province"]="未知";
	}
	return $rearr;
}
...
function liuliang_fanhuandxnum_daili_llsendtool_ty($tid){
	global $con;
	$time=time()-60*60*24*15;
	$sql="select * from `liuliangdaili_log` where id=".$tid." and createtime>".$time." and zt=3 and istk=0";
	$re=$con->query($sql);
	...
	//更新返款状态
	$sql="UPDATE liuliangdaili_log set istk=1,zt=2,upzttime=".time()." where id=".$orderinfo["id"];
	$re=$con->query($sql);
